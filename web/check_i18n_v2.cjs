...
async function shot(browser, token, url, name) {
  ...
  await page.screenshot({ path: `screenshots/i18n_v2_${name}.png` });
  await page.close();
...
(async () => {
  ...
  await shot(browser, token, '/', 'dashboard');
  ...
  await shot(browser, token, '/agents', 'agents_hub');
  await shot(browser, token, '/agentix', 'agentix');
  await shot(browser, token, '/query-center', 'query_center');
