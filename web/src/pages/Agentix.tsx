...
function renderMarkdown(text: string): string {
  ...
  const rendered = parts.map((part, i) => {
    if (i % 2 === 1) {
      ...
      return (
        ...
        `<button data-copy-target="${blockId}" onclick="(function(btn){` +
          ...
          `})}` +
        `})(this)" style="position:absolute;top:6px;right:6px;background:#2d3452;border:none;color:#94a3b8;font-size:10px;padding:2px 8px;border-radius:3px;cursor:pointer;z-index:1">复制</button>` +
    ...
    s = s.replace(/^- (.+)$/gm, '<li style="margin:2px 0;color:#cbd5e1">$1</li>');
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 6, alignSelf: 'flex-start' }}>
      {questions.map(q => (
        <button
          ...
          onMouseEnter={e => {
            (e.currentTarget as HTMLButtonElement).style.background = '#1e2860';
            (e.currentTarget as HTMLButtonElement).style.borderColor = '#6366f1';
          }}
          onMouseLeave={e => {
            (e.currentTarget as HTMLButtonElement).style.background = '#161c2d';
            (e.currentTarget as HTMLButtonElement).style.borderColor = '#3b4268';
          }}
        >
          {q}
        </button>
...
function 对话Bubble({ msg, onEdit, onSave, onFollowUp }: 对话BubbleProps) {
  ...
  if (msg.role === 'user') {
    return (
      ...
      >
        {msg.hasContext && (
          <div style={{ fontSize: 10, background: '#1e2860', border: '1px solid #3b4268', borderRadius: 10, padding: '2px 8px', color: '#818cf8', marginBottom: 2, alignSelf: 'flex-end' }}>
            📌 上下文已附加
          </div>
        )}
        <div style={{ background: '#1e2860', border: '1px solid #3b4268', borderRadius: '10px 10px 2px 10px', padding: '10px 14px', fontSize: 12.5, color: '#cbd5e1', lineHeight: 1.5 }}>
          {msg.text}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {timeLabel && (
            <span style={{ fontSize: 10, color: '#475569' }}>{timeLabel}</span>
          )}
  ...
  if (msg.html) {
    return (
      ...
      >
        <div
          style={{ background: 'var(--bg-secondary)', border: '1px solid #2d3452', borderRadius: '10px 10px 10px 2px', padding: '10px 14px', fontSize: 12.5, color: '#cbd5e1', lineHeight: 1.5 }}
    ...
    >
      ...
      >
        {msg.streaming ? (
          <>
            {msg.text}
            <span style={{ display: 'inline-block', width: 2, height: 14, background: '#a5b4fc', marginLeft: 2, verticalAlign: 'text-bottom', animation: 'agentix-cursor-blink 0.8s infinite' }} />
          </>
        ) : (
...
function ActionBtn({ onClick, title, children }: { onClick: (e: React.MouseEvent) => void; title: string; children: React.ReactNode }) {
  return (
    <button
      ...
      onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = '#2d3452'; }}
      onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = '#1e2233'; }}
    >
      {children}
    </button>
  );
  ...
  return (
    <div style={{ position: 'relative', display: 'inline-block' }}>
      <button
        onClick={() => setOpen(v => !v)}
        title="快捷键帮助"
        style={{ background: 'none', border: '1px solid #2d3452', color: '#64748b', fontSize: 11, padding: '2px 7px', borderRadius: 4, cursor: 'pointer', lineHeight: 1.6 }}
      >
        ？
      </button>
      {open && (
        ...
        >
          <div style={{ fontWeight: 600, color: '#e2e8f0', marginBottom: 8, fontSize: 12 }}>快捷键</div>
          {[
            ['Enter', '发送消息'],
            ['Shift+Enter', '换行'],
            ['Ctrl+K', '清空对话'],
            ...
            <div key={key} style={{ display: 'flex', justifyContent: 'space-between', gap: 16, marginBottom: 5 }}>
              <kbd style={{ background: '#2d3452', border: '1px solid #3b4268', borderRadius: 3, padding: '1px 5px', fontSize: 10, color: '#a5b4fc', fontFamily: 'monospace' }}>{key}</kbd>
              <span>{desc}</span>
            </div>
          ))}
        </div>
...
function SavedModal({ item, onClose }: SavedModalProps) {
  return (
    <div
      style={{ position: 'fixed', inset: 0, zIndex: 400, background: 'rgba(0,0,0,.6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      onClick={onClose}
    >
      <div
        style={{ background: '#1a1f2e', border: '1px solid #2d3452', borderRadius: 10, padding: '20px 24px', maxWidth: 560, width: '90%', maxHeight: '70vh', overflow: 'auto', position: 'relative' }}
        onClick={e => e.stopPropagation()}
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <span style={{ fontSize: 11, color: '#475569', background: '#1e2233', borderRadius: 10, padding: '2px 10px' }}>{formatTimestamp(item.timestamp)}</span>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#64748b', fontSize: 18, cursor: 'pointer' }}>✕</button>
        </div>
        <div style={{ fontSize: 12.5, color: '#cbd5e1', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{item.fullText}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      <div style={{
        ...
        height: '100%',
        background: '#0d1019',
        borderLeft: '1px solid #1e2233',
        display: 'flex',
        flexDirection: 'column',
        flexShrink: 0,
      }}>
        {/* Sidebar header */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderBottom: '1px solid #1e2233', flexShrink: 0 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: '#94a3b8' }}>📚 已保存分析 ({savedList.length})</span>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {savedList.length === 0 && (
            <div style={{ padding: '16px 14px', fontSize: 11, color: '#475569', fontStyle: 'italic', textAlign: 'center' }}>暂无保存的分析</div>
          )}
          {savedList.slice(0, 20).map(item => (
            <div
              key={item.id}
              style={{ padding: '8px 14px', borderBottom: '1px solid #1a1f30', cursor: 'pointer' }}
              onClick={() => setModalItem(item)}
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 }}>
                <span style={{ fontSize: 10, background: '#1e2233', borderRadius: 8, padding: '1px 7px', color: '#64748b' }}>{formatTimestamp(item.timestamp)}</span>
                <button
                  onClick={e => { e.stopPropagation(); onDelete(item.id); }}
                  title="删除"
                  style={{ background: 'none', border: 'none', color: '#475569', fontSize: 13, cursor: 'pointer', padding: '0 2px', lineHeight: 1 }}
                >
                  🗑️
                </button>
              </div>
              <div style={{ fontSize: 11, color: '#94a3b8', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as const }}>
        ...
        {/* Clear all button */}
        {savedList.length > 0 && (
          <div style={{ padding: '10px 14px', borderTop: '1px solid #1e2233', flexShrink: 0 }}>
            <button
              onClick={onClearAll}
              style={{ width: '100%', background: '#1e2233', border: '1px solid #2d3452', color: '#94a3b8', fontSize: 11, padding: '6px', borderRadius: 5, cursor: 'pointer' }}
            >
              清空全部
            </button>
          </div>
...
function ConvHistorySidebar({
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{ padding: '12px 12px 8px', borderBottom: '1px solid #1e2233', flexShrink: 0 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: '#64748b', letterSpacing: '.06em', marginBottom: 8 }}>对话历史</div>
        <button
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {filtered.length === 0 && (
          <div style={{ padding: '14px 12px', fontSize: 11, color: '#475569', fontStyle: 'italic', textAlign: 'center' }}>
        ...
        {filtered.map(conv => (
          <div
            ...
            style={{
              ...
              cursor: 'pointer',
              background: activeConvId === conv.id ? '#1e2233' : hoveredId === conv.id ? '#131826' : 'transparent',
              display: 'flex',
          ...
          >
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{
                fontSize: 11,
                color: activeConvId === conv.id ? '#a5b4fc' : '#94a3b8',
              ...
              <div style={{ fontSize: 10, color: '#475569' }}>{relativeTime(conv.createdAt)}</div>
            </div>
...
export default function Agentix() {
  ...
  const charCountColor = charCount > 1800 ? '#ef4444' : charCount > 1500 ? '#f59e0b' : '#475569';
  ...
  const panelTabStyle = (key: PanelTab): React.CSSProperties => ({
    ...
    color: activePanelTab === key ? '#a5b4fc' : '#64748b',
    borderColor: activePanelTab === key ? '#3b4268' : '#2d3452',
  });
        ...
        <div style={{ width: chatColumnWidth, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderBottom: '1px solid #1e2233', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              ...
              <select
                ...
                title="注入上下文"
                style={{ background: '#1e2233', border: '1px solid #2d3452', color: contextMode === 'none' ? '#64748b' : '#a5b4fc', fontSize: 11, padding: '3px 8px', borderRadius: 4, cursor: 'pointer', outline: 'none' }}
              >
              ...
              {contextMode !== 'none' && (
                <button
                  ...
                  title="清除上下文"
                  style={{ background: 'none', border: 'none', color: '#64748b', fontSize: 10, padding: '0', cursor: 'pointer', whiteSpace: 'nowrap', textDecoration: 'underline', textUnderlineOffset: 2 }}
                >
                  清除上下文
                </button>
              )}
              ...
              <button
                ...
                title="已保存分析"
                style={{ background: savedOpen ? '#2d3452' : 'none', border: '1px solid #2d3452', color: savedOpen ? '#a5b4fc' : '#64748b', fontSize: 11, padding: '3px 8px', borderRadius: 4, cursor: 'pointer', whiteSpace: 'nowrap' }}
              >
                📚{savedList.length > 0 ? ` ${savedList.length}` : ''}
              </button>
              <button
                onClick={handleClearHistory}
                style={{ background: 'none', border: '1px solid #2d3452', color: '#64748b', fontSize: 11, padding: '3px 10px', borderRadius: 4, cursor: 'pointer', whiteSpace: 'nowrap' }}
              >
                清空对话
              </button>
          ...
          <div style={{ display: 'flex', gap: 4, padding: '10px 18px 0', flexShrink: 0 }}>
            ...
            <button
              ...
              style={{
                ...panelTabStyle('xql'),
                color: activePanelTab === 'xql' ? '#34d399' : '#64748b',
                borderColor: activePanelTab === 'xql' ? '#065f46' : '#2d3452',
                background: activePanelTab === 'xql' ? '#0d2318' : 'var(--bg-card)',
              }}
            >
              ⚡ XQL转换
            </button>
          ...
          {activePanelTab === 'xql' ? (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '18px', overflowY: 'auto', gap: 14 }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: xqlLoading || !xqlInput.trim() ? '#1e2233' : 'linear-gradient(135deg, #065f46, #059669)',
                  border: '1px solid #065f46',
                  color: xqlLoading || !xqlInput.trim() ? '#475569' : '#fff',
                  fontSize: 12,
                  padding: '7px 20px',
                  borderRadius: 6,
                  cursor: xqlLoading || !xqlInput.trim() ? 'not-allowed' : 'pointer',
                  fontWeight: 500,
                  display: 'flex',
                  alignItems: 'center',
                  gap: 6,
              ...
              {/* Error */}
              {xqlError && (
                <div style={{ background: '#1c0a0a', border: '1px solid #7f1d1d', borderRadius: 6, padding: '10px 12px', fontSize: 12, color: '#fca5a5', lineHeight: 1.5 }}>
                  ⚠️ {xqlError}
                </div>
              )}
              ...
              {xqlResult && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: '#64748b', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span>生成的 XQL</span>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button
                        onClick={handleXqlCopyToClipboard}
                        style={{
                          background: xqlCopied ? '#065f46' : '#1e2233',
                          border: '1px solid #2d3452',
                          color: xqlCopied ? '#34d399' : '#94a3b8',
                          fontSize: 10.5,
                          padding: '3px 10px',
                          borderRadius: 4,
                          cursor: 'pointer',
            ...
            <>
              ...
              <div ref={chatScrollRef} style={{ flex: 1, overflowY: 'auto', padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                {contextMode === 'alerts' && (
                  <div style={{ background: '#0d1f2d', border: '1px solid #0c4a6e', borderRadius: 8, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: '#38bdf8', display: 'flex', alignItems: 'center', gap: 6 }}>
                      🤖 <span>AI 分析摘要</span>
                      <span style={{ fontSize: 10, color: '#475569', fontWeight: 400, marginLeft: 'auto' }}>基于最近告警</span>
                    </div>
                    {!alertSummaryKey ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <button
                          onClick={() => fetchAlertSummary('latest')}
                          style={{ background: '#0c4a6e', border: '1px solid #0369a1', color: '#7dd3fc', fontSize: 11, padding: '5px 14px', borderRadius: 5, cursor: 'pointer', fontWeight: 500 }}
                        >
                          获取 AI 摘要
                        </button>
                        <span style={{ fontSize: 11, color: '#475569' }}>点击获取当前告警的 AI 智能分析</span>
                      </div>
                    ) : alertSummaryLoading ? (
                      ...
                      <div style={{ fontSize: 11, color: '#fca5a5' }}>⚠️ {alertSummaryError}</div>
                    ) : alertSummaryKey && alertSummaryCache[alertSummaryKey] ? (
                      <div style={{ fontSize: 12.5, color: '#bae6fd', lineHeight: 1.65, background: '#082f49', borderRadius: 6, padding: '10px 12px', whiteSpace: 'pre-wrap' }}>
              ...
              <div style={{ padding: '14px 18px', borderTop: '1px solid #1e2233', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8 }}>
                  <div style={{ flex: 1, position: 'relative' }}>
                    <textarea
                      ...
                      rows={1}
                      disabled={loading}
                      style={{ width: '100%', boxSizing: 'border-box', background: '#1e2233', border: '1px solid #2d3452', borderRadius: 8, color: '#e2e8f0', fontSize: 12.5, padding: '9px 12px', resize: 'none', outline: 'none', fontFamily: 'inherit', lineHeight: 1.4, height: 40, transition: 'border-color .2s', opacity: loading ? 0.6 : 1 }}
                    />
                  </div>
                  ...
                  <button
                    ...
                    title="附加文件"
                    style={{ background: '#1e2233', border: '1px solid #2d3452', color: '#64748b', width: 36, height: 36, borderRadius: 7, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 16 }}
                  >
                    📎
                  </button>
                  ...
                  <button
                    onClick={handle発送}
                    disabled={loading}
                    style={{ background: '#6366f1', border: 'none', color: '#fff', width: 36, height: 36, borderRadius: 7, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, opacity: loading ? 0.6 : 1 }}
                  >
                ...
                {contextLoaded && contextLoaded.mode !== 'none' && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
                    <span style={{ fontSize: 10, background: '#052e16', border: '1px solid #166534', borderRadius: 10, padding: '2px 8px', color: '#4ade80', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                      ✓ 已加载 {contextLoaded.count} 条上下文
                    </span>
                    <button
                      onClick={() => { setContextMode('none'); setContextLoaded(null); }}
                      style={{ background: 'none', border: 'none', color: '#64748b', fontSize: 10, padding: 0, cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 2 }}
                    >
                      清除上下文
                    </button>
                  </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 5 }}>
                  ...
                  <select
                    ...
                    style={{ background: '#1e2233', border: '1px solid #2d3452', color: '#64748b', fontSize: 10, padding: '2px 6px', borderRadius: 4, cursor: 'pointer', outline: 'none' }}
                  >
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
                    {COPILOT_CHIPS.map(chip => (
                      <button
                        ...
                        onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = '#2d3452'; (e.currentTarget as HTMLButtonElement).style.color = '#94a3b8'; }}
                      >
                        {chip}
                      </button>
                    ))}
                ...
                <div style={{ marginTop: 8 }}>
                  <div style={{ fontSize: 10, color: '#475569', fontWeight: 500, marginBottom: 5, letterSpacing: '.04em' }}>快速分析</div>
                  <div
                    style={{
                      display: 'flex',
                      gap: 6,
