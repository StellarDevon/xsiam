...
STATIC bool wzcp_handshake_to_gateway(int server_id);
...
void start_agent(int is_startup)
{
    ...
    if (xsiam_wzcp_enabled()) {
        ...
        while (1) {
            if (wzcp_handshake_to_gateway(current_server_id)) {
                return;
            }
            ...
            if (agt->server[current_server_id + 1].rip) {
                ...
                current_server_id = 0;
                mwarn("Unable to connect to any WZCP gateway.");
            }
        }
    }
...
STATIC bool wzcp_handshake_to_gateway(int server_id)
{
    ...
    unsigned char reply[OS_SIZE_1024];
    ...
    agent_id = xsiam_wzcp_agent_uuid();
    agent_name = keys.keyentries[0]->name ? keys.keyentries[0]->name : "unknown";
    host_type = xsiam_wzcp_host_type();
    xsiam_wzcp_collect_mac_addresses(mac_addresses, sizeof(mac_addresses));

    body_len = xsiam_wzcp_build_hello_ex(body, sizeof(body), agent_id,
                                         agent_name, __ossec_version,
                                         host_type, mac_addresses);
    if (body_len < 0) {
        merror("Unable to build WZCP hello.");
        return false;
    }

    if (xsiam_wzcp_send_frame(agt->sock, XSIAM_WZCP_MSG_HELLO,
                              (uint32_t) atoi(agent_id),
                              xsiam_wzcp_next_seq(),
                              body, (uint32_t) body_len) < 0) {
        merror("Unable to send WZCP hello.");
        return false;
    }

    if (xsiam_wzcp_recv_frame(agt->sock, &header, reply, sizeof(reply)) < 0 ||
        header.msg_type != XSIAM_WZCP_MSG_ACK ||
        xsiam_wzcp_parse_ack(reply, header.body_len, &ack_seq) < 0) {
        merror("Unable to receive WZCP hello ACK.");
        return false;
    }

    available_server = (int) time(NULL);
    w_agentd_state_update(UPDATE_ACK, (void *) &available_server);
    minfo("Connected to WZCP gateway ([%s]:%d/%s).",
          agt->server[server_id].rip,
          agt->server[server_id].port,
          agt->server[server_id].protocol == IPPROTO_UDP ? "udp" : "tcp");
