...
#ifdef WIN32
static int wzcp_next_heartbeat_interval(void)
{
    static int seeded = 0;
...
static void wzcp_schedule_heartbeat(time_t *next_heartbeat)
{
    *next_heartbeat = time(NULL) + wzcp_next_heartbeat_interval();
}

static int wzcp_send_heartbeat_if_due(time_t *next_heartbeat, int *last_sock)
{
    time_t now;
    uint32_t agent_id;

    if (!xsiam_wzcp_enabled() || agt->sock == -1) {
        *next_heartbeat = 0;
        *last_sock = -1;
        return 0;
    }

    if (*last_sock != agt->sock || *next_heartbeat == 0) {
        *last_sock = agt->sock;
        wzcp_schedule_heartbeat(next_heartbeat);
        return 0;
    }
    ...
    agent_id = keys.keyentries[0] ? (uint32_t)atoi(keys.keyentries[0]->id) : 0;
    if (xsiam_wzcp_send_frame(agt->sock, XSIAM_WZCP_MSG_HEARTBEAT,
                              agent_id, xsiam_wzcp_next_seq(),
                              NULL, 0) < 0) {
        return -1;
    }

    mdebug2("Sent WZCP HEARTBEAT to gateway.");
    wzcp_schedule_heartbeat(next_heartbeat);
    return 0;
}
#endif
...
int receive_msg()
{
    ...
    if (xsiam_wzcp_enabled()) {
        unsigned char body[OS_SIZE_2048];
        struct xsiam_wzcp_header header;
        uint64_t ack_seq = 0;

        if (xsiam_wzcp_recv_frame(agt->sock, &header, body, sizeof(body)) < 0) {
            return -1;
        }

        if (header.msg_type == XSIAM_WZCP_MSG_ACK) {
            if (xsiam_wzcp_parse_ack(body, header.body_len, &ack_seq) == 0) {
                available_server = (int)time(NULL);
                w_agentd_state_update(UPDATE_ACK, (void *) &available_server);
                mdebug2("Received WZCP ACK for seq '%llu'.",
                        (unsigned long long) ack_seq);
            }
            return 0;
        }

        if (header.msg_type == XSIAM_WZCP_MSG_HEARTBEAT) {
            unsigned char ack[8];
            int ack_len = xsiam_wzcp_build_ack(ack, sizeof(ack), header.seq);
            if (ack_len > 0) {
                xsiam_wzcp_send_frame(agt->sock, XSIAM_WZCP_MSG_ACK,
                                      (uint32_t) atoi(keys.keyentries[0]->id),
                                      xsiam_wzcp_next_seq(), ack,
                                      (uint32_t) ack_len);
            }
            return 0;
        }

        if (header.msg_type == XSIAM_WZCP_MSG_CONTROL && header.body_len > 0) {
            body[header.body_len < sizeof(body) ? header.body_len : sizeof(body) - 1] = '\0';
            mdebug2("Received WZCP control message: '%s'", body);
            return 0;
        }
...
{
    int rc = 0;
    time_t next_wzcp_heartbeat = 0;
    int last_wzcp_sock = -1;
    ...
    while (1) {
        ...
        if (wzcp_send_heartbeat_if_due(&next_wzcp_heartbeat, &last_wzcp_sock) < 0) {
            w_agentd_state_update(UPDATE_STATUS, (void *) GA_STATUS_NACTIVE);
            merror(LOST_ERROR);
            os_setwait();
            start_agent(0);
